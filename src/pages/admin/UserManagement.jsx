                                        ...
                                        <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <button 
                                                    ...
                                                    className={`relative inline-flex items-center px-4 py-1.5 rounded-full text-xs font-bold transition-all shadow-sm ${isPaid ? 'bg-green-100 text-green-700 hover:bg-green-200 border border-green-200' : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100 border border-yellow-200'} ${updatingId === user.id ? 'opacity-50 cursor-wait' : ''}`}
                                                >
                                                    {updatingId === user.id ? <i className="fas fa-circle-notch fa-spin mr-2"></i> : <i className={`fas ${isPaid ? 'fa-check-circle' : 'fa-clock'} mr-2`}></i>}
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <button onClick={() => viewHistory(user.id)} className="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors font-bold text-xs">Ver Historial</button>
                                            </td>
                                        </tr>
                                    );
                                })
            ...
            {selectedUserHistory && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-2xl animate-fade-in">
                        ...
                        <button onClick={() => setSelectedUserHistory(null)} className="mt-6 w-full bg-gray-800 text-white py-3 rounded-xl hover:bg-gray-900 font-medium transition-colors">Cerrar</button>
                    </div>
                </div>
            )}
        </div>
